...
double linNegLogLikelihoodCalc(int *nrow, double *eta, double *y)
{
  ...
  void linSolver(double *X, double *y, int* index, int *nrow, int *ncol, int *numGroup, double *beta, int *rangeGroupInd, int *groupLen, double *weights, double *lambda1, double *lambda2, int *innerIter, double *thresh, double *ldot, double *nullBeta, double *gamma, double *eta, int* betaIsZero, int& groupChange, int* isActive, int* useGroup, double *step, int *reset)
{
  ...
  for(int i = 0; i < numGroup[0]; i++)
      ...
      for(int j = 0; j < groupLen[i]; j++)
	{
      ...
      if(zeroCheck <= pow(lambda2[0] * weights[i],2))  //Or not?
	{
	  if(betaIsZero[i] == 0)
	    {
	      for(int k = 0; k < nrow[0]; k++)
      ...
      else
	{
	  ...
	  while(count <= innerIter[0] && check > thresh[0])
	      ...
	      while(diff < 0)
		{
		  ...
		  if(norm != 0){
		    uOp = (1 - lambda2[0]*weights[i]*t/norm);   //Or not?
		  }
		  else{uOp = 0;}
  ...
  int linNest(double *X, double* y, int* index, int *nrow, int *ncol, int *numGroup, int *rangeGroupInd, int *groupLen, double *weights, double *lambda1, double *lambda2, double *beta, int *innerIter, int *outerIter, double *thresh, double *outerThresh, double *eta, double *gamma, int *betaIsZero, double *step, int *reset)
{
  double* prob = NULL;
   ...
   while(groupChange == 1)
     {
       ...
       linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, weights, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, isActive, useGroup, step, reset);
  ...
  while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      ...
      for(int i = 0; i < numGroup[0]; i++)
	{
      ...
      linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, weights, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, isActive, tempIsActive, step, reset);
  ...
  void logitSolver(double *X, int *y, int* index, int *nrow, int *ncol, int *numGroup, double *beta, int *rangeGroupInd, int *groupLen, double *weights, double *lambda1, double *lambda2, int *innerIter, double *thresh, double *ldot, double *nullBeta, double *gamma, double *eta, int* betaIsZero, int& groupChange, int* isActive, int* useGroup, double *prob, double *betaZero, double *step)
{
  double *theta = new double[ncol[0]];
  ...
  int logitNest(double *X, int* y, int* index, int *nrow, int *ncol, int *numGroup, int *rangeGroupInd, int *groupLen, double *weights, double *lambda1, double *lambda2, double *beta, int *innerIter, int *outerIter, double *thresh, double *outerThresh, double *eta, double *gamma, int *betaIsZero, double* betaZero, double *step)
{
   ...
   while(groupChange == 1)
     {
       ...
       logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, weights, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, isActive, useGroup, prob, betaZero, step);
  ...
  while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      ...
      for(int i = 0; i < numGroup[0]; i++)
	{
      ...
      logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd, groupLen, weights, lambda1, lambda2, innerIter, thresh, ldot, nullBeta, gamma, eta, betaIsZero, groupChange, tempIsActive, isActive, prob, betaZero, step);
